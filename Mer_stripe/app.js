...
app.post('/login', async function (req, res) {
  ...
  customer = await UserService.getUserByEmail(email)
  if (!customer) {
    res.redirect('/')
  } else {
    res.render('account.ejs', { customer })
...
app.post('/webhook', async (req, res) => {
  let event
  console.log("webhook successful!!!!!", req.body)
  try {
    event = Stripe.createWebhook(req.body, req.header('Stripe-Signature'))
  } catch (err) {   
    console.log("error!!!" , err, "error_end!!!!!") 
  ...
  switch (event.type) {
    ...
    case 'customer.subscription.created': {
      ...
      if (isOnTrial) {
        if(user) {  
          user.hasTrial = true
          user.endDate = new Date(Number(data.current_period_end) * 1000)
        }
      } else if (data.status === 'active') {
        console.log(data.current_period_end);
        try {
          if(user)  user.endDate = new Date(Number(data.current_period_end) * 1000)
        } catch (err) {   
          console.log("error!!!" , err, "error_end!!!!!") 
          return res.sendStatus(400)
        }
